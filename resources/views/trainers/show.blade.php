                ...
                <div class="grid justify-items-stretch pt-2 pr-2 w-full" >
                    ...
                    <div class="card card-border card-compact lg:card-normal w-full">
                        ...
                        <div class="flex flex-col w-full items-center">               
                            <figure>
                                <img src="/storage/{{ $trainer->image }}"  width="900px" image-center>
                            </figure> 
                        </div>
                        @endif
                        <div class="card-body mt-4 ">
                            
                            <h2 class="card-title font-bold text-2xl pb-3  text-center"  style="color: #F25C05">{{ $trainer->imie}} {{ $trainer->nazwisko}}</h2> 
                            @if ( $trainer->specializations)
                                <span class="flex justify-end text-sm text-gray-600">
                                    @foreach ( $trainer->specializations as  $trainer->specialization)
                                        <span
                                            class="mr-2 rounded px-2.5 py-0.5 text-xs font-semibold text-gray-800 dark:bg-gray-700 dark:text-gray-300" style="background-color: #F2DF80">{{  $trainer->specialization->name }}</span>
                                    @endforeach
                                </span>
                            @endif
                            <p class="text-black text-center font-semibold text-2xl">{{ $trainer->opis}}</p>
                        </div>
                    </div>
                </div>
